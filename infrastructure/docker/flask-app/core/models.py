"""
Database models for REIT Newsletter System
...
class Company(Base):
    ...
    # SEC EDGAR identifiers
    cik = Column(String(10))  # SEC Central Index Key (10 digits, leading zeros)
    op_cik = Column(String(10))  # Operating Partnership CIK (if applicable)
    op_name = Column(String(200))  # OP legal name (e.g., "Simon Property Group, L.P.")
    op_has_unique_filings = Column(Boolean, default=False)  # True if OP publishes content not in REIT filings
...
class PressRelease(Base):
    ...
    newsletter_status = Column(String(20), default='ready', index=True)  # 'ready', 'needs_review', 'published', 'excluded'
    newsletter_section = Column(String(20), nullable=True)  # Manual override: 'headline', 'other', or NULL (auto)
    ...
    def generate_unique_id(self, db_session):
        ...
        SECURITY: Uses secrets module (not random.randint) to prevent ID enumeration.
        """
        import secrets
        while True:
            # Generate URL-safe, cryptographically secure ID
