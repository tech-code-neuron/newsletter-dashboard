...
# Copy other top-level .py modules (like metadata_builder.py)
for py_file in "$SCRIPT_DIR"/*.py; do
    filename=$(basename "$py_file")
    # Skip handler.py (already copied), test files, and non-essential files
    case "$filename" in
        handler.py|handler_old.py|handler_new.py|deploy_direct.py|typing_extensions.py|models.py|date_extraction.py)
            continue
            ;;
        *)
            if [ -f "$py_file" ]; then
